...
<main>
  ...
  <div class="container">
    <h1 class="text-light text-center mt-2"><strong>Editar usuario</strong></h1>
      ...
      <form class="col-4" action="index.php?c=usuario&a=actualizar" method="POST">
        <fieldset>
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Ej. Juan" autocomplete="off" value="<?php echo $usuario['nombre'];?>">
          </div>
          <div class="form-group">
            <label for="apellido_p" class="form-label mt-2">Apellido Paterno</label>
            <input type="text" class="form-control" id="apellido_p" name="apellido_p" placeholder="Ej. Salazar" autocomplete="off" value="<?php echo $usuario['apellido_p'];?>">
          </div>
          <div class="form-group">
            <label for="apellido_m" class="form-label mt-2">Apellido Materno</label>
            <input type="text" class="form-control" id="apellido_m" name="apellido_m" placeholder="Ej. Corona" autocomplete="off" value="<?php echo $usuario['apellido_m'];?>">
          </div>
          <div class="form-group">
            <label for="sexo" class="form-label mt-2">Sexo</label>
            <select class="form-select" id="sexo" name="sexo">
              <option selected disabled>-Seleccionar-</option>
              <option <?php if($usuario['sexo'] == 'F') echo 'selected';?> >Femenino</option>
              <option <?php if($usuario['sexo'] == 'M') echo 'selected';?> >Masculino</option>
            </select>
          </div>
          <div class="form-group">
            <label for="fecha_nac" class="form-label mt-2">Fecha Nacimiento</label>
            <input type="date" class="form-control" id="fecha_nac" name="fecha_nac" min="1950-01-01" max="<?php $actual = date('Y-m-d'); $nueva = strtotime ('-12 year' , strtotime($actual)); echo date ('Y-m-d', $nueva);?>" value="<?php echo $usuario['fecha_nac'];?>">
          </div>
          <div class="form-group">
            <label for="peso" class="form-label mt-2">Peso</label>
            <input type="number" min="30" max="200" class="form-control" id="peso" name="peso" placeholder="Ej. 55" autocomplete="off" value="<?php echo $usuario['peso'];?>">
          </div>
          <div class="form-group">
            <label for="altura" class="form-label mt-2">Altura</label>
            <input type="number" min="1.00" max="2.30" step="0.01" class="form-control" id="altura" name="altura" placeholder="Ej. 1.60" autocomplete="off" value="<?php echo $usuario['altura'];?>">
          </div>
          <div class="form-group">
            <label for="domicilio" class="form-label mt-2">Domicilio</label>
            <input type="text" class="form-control" id="domicilio" name="domicilio" placeholder="Ej. Av. Revolución 1500" autocomplete="off" value="<?php echo $usuario['domicilio'];?>">
          </div>
          <div class="form-group">
            <label for="telefono" class="form-label mt-2">Telefono</label>
            <input type="number" min="0" class="form-control" id="telefono" name="telefono" placeholder="Ej. 3310940910" autocomplete="off" value="<?php echo $usuario['telefono'];?>">
          </div>
          <div class="form-group">
